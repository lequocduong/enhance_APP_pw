...
typedef	unsigned long long	GUInt64;
#elif	WIN32
typedef	__int64				GInt64;		//	ll
typedef	unsigned __int64	GUInt64;	//	ull
#endif
//----------------

typedef	signed char			GInt8;		//	c
typedef	unsigned char		GUInt8;		//	by
typedef	short				GInt16;		//	s
typedef	unsigned short		GUInt16;	//	us

/*目前主流编译器(不论是否64位平台)int都是32位,只有老式编译器如turboC为16位, */
typedef int					GInt32;		//	l
typedef unsigned int		GUInt32;	//	ul


typedef float				GFloat;		//	f
typedef double				GDouble;	//	d
typedef GUInt8				GByte;		//	by
typedef GUInt16				GWord;
typedef GUInt32				GDWord;
typedef void*				GHandle;	//	h
typedef char				GChar;		//	c
typedef int 				GBool;		//	b
typedef void				GVoid;		//	v
typedef void*				GPVoid;
